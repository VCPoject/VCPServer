...
public class WeeklyStats extends Controller {
	...
	public void getRowData(){
		...
		{
			i++;
			impOrder[j]=Integer.parseInt(getResult().get(i).toString()) ;
			hitpalgutimpOrder[Integer.parseInt(getResult().get(i++).toString())/10]++;
			cancelOrder[j]=Integer.parseInt(getResult().get(i).toString());
			hitpalgutcancelOrder[Integer.parseInt(getResult().get(i++).toString())/10]++;
			memberCount[j]=Integer.parseInt(getResult().get(i).toString());
			hitpalgutmemberCount[Integer.parseInt(getResult().get(i++).toString())/10]++;
			memHasMorCar[j]=Integer.parseInt(getResult().get(i).toString());
			hitpalgutmemHasMorCar[Integer.parseInt(getResult().get(i++).toString())/10]++;
			lateCount[j]=Integer.parseInt(getResult().get(i).toString());
			hitpalgutlateCount[Integer.parseInt(getResult().get(i++).toString())/10]++;
			j++;
		}
	...
	*/
	public Vector<Vector<Object>> toVector() {
		...
		Object[] obj ={"SELECT avg,median,decile1,decile2,decile3,decile4,decile5,decile6,decile7,decile8,decile9,decile10"
				+ " FROM vcp_employ.weekly_reports ORDER BY date;"};
		sendQueryToServer(obj);
		int j=1;
		for(int i=0;i<getResult().size();i++){
